...
def train_classifier():
    data_dir="C:/Users/admin/Documents/e-voting/data"
...
def detect_face():
    def draw_boundary(img,classifier,scaleFactor,minNeighbors,color,text,clf):
        ...
        for(x,y,w,h) in features:
            ...
            mycursor.execute("select Name from sample where id="+str(id))
            s = mycursor.fetchone()
            s = ''+''.join(s)
...
def generate_dataset():
    if(t1.get()=="" or t2.get()=="" or t3.get()==""):
        ...
        for x in myresult:
            id+=1
        sql="insert into sample(id,Name,Age,Address) values(%s,%s,%s,%s)"
