...
admin_lable=Label(root,text="Admin Login",fg="Black",font="Rockwell 22 bold")
admin_lable.place(x=278,y=110)

frame1=Frame(root,background="Light Blue",height=350,width=540)
frame1.place(x=82,y=170)
...
def login_form():
    ...
    if username=="Admin" and password=="123456":
       messagebox.showinfo("Login Successful","Login Successful!",parent=root)
       root.destroy()
       call(["python","eAdmindash.py"]) 
    if username=="" or password=="":
       messagebox.showwarning("no entry","Please enter Username or Password",parent=root)
    elif username!="Admin" or password!="123456":
       messagebox.showerror("invalid entry","Invalid Username or Password",parent=root)    
   
log= Button(frame1,text="Log In", foreground="White",background="Blue",font="Candara 16 bold",border=0,bd=2,command=login_form)
